...
int gsl_linalg_solve_tridiag(const std::vector<double> & diag, const std::vector<double> & abovediag,
	...
	if (diag.size() != rhs.size()) {
		std::cout << "size of diag must match rhs\n";
		exit(GSL_EBADLEN);
	} else if (abovediag.size() != rhs.size() - 1) {
		std::cout << "size of abovediag must match rhs-1\n";
		exit(GSL_EBADLEN);
	} else if (belowdiag.size() != rhs.size() - 1) {
		std::cout << "size of belowdiag must match rhs-1\n";
		exit(GSL_EBADLEN);
	} else if (solution.size() != rhs.size()) {
		std::cout << "size of solution must match rhs\n";
		exit(GSL_EBADLEN);
	} else {
