...
Particle TimeEvolutor::evolve(const Sources& Q, const DiffusionCoefficient& D_zz,
	...
	_z = Q.getAxisZ();
	_E = Q.getAxisT();
	_p = build_momentum_vector(_E, pid);
	size_t counter = 0;
	size_t idisc = _z.getNearestIndex(0.);
	Particle f(_E, _z, pid, 1. / cgs::cm_3 / cgs::GeV_c_3);
	auto I_Q = Q.integrate();
	while (counter < _max_counter) {
		if (evolveInSpace)
			evolve_in_z(Q, D_zz, u_z, f, num_operators());
		if (evolveInMomentum)
			evolve_in_p(Q, D_pp, dpdt, u_z, f, num_operators());
		if (counter % 1000 == 0) {
...
void TimeEvolutor::evolve_in_p(const Sources& Q, const MomentumDiffusionCoefficient& D_pp, const EnergyLosses& dpdt,
		const AdvectionVelocity& u, Particle& f, const size_t& number_of_operators) {
	double dt_half = 0.5 * _dt;
	double dz = _z.getAverageStep();
	size_t p_size = _p.size(), z_size = _z.size();
	...
	for (size_t iz = 1; iz < z_size - 1; ++iz) {
		...
		for (size_t ip = 1; ip < p_size - 1; ++ip) {
			double b_i = dudz / 3.0 / (_p.get(ip + 1) / _p.get(ip) - 1.0);
			double Lp = 0, Cp = 0, Up = 0;
			Cp += b_i;
			Up += b_i;

			Cp += dpdt.get(ip, iz) / _p.get(ip) * (1. / (_p.get(ip + 1) / _p.get(ip) - 1.0) - 2.);
			Up += dpdt.get(ip + 1, iz) / (_p.get(ip + 1) - _p.get(ip));

			double p_c = _p.get(ip + 1) - _p.get(ip - 1);
			double p_up = _p.get(ip + 1) - _p.get(ip);
			double p_do = _p.get(ip) - _p.get(ip - 1);
			double Dpp_c = D_pp.get(ip, iz);
			double Dpp_up = D_pp.get(ip + 1, iz);
			double Dpp_do = D_pp.get(ip - 1, iz);
			double p = _p.get(ip);

			Lp += 2. * Dpp_c / pow2(p_c) * (p_c / p_up + 0.5 * (Dpp_up - Dpp_do) / Dpp_c + p_c / p);
			Cp += 2. * Dpp_c / pow2(p_c) * (p_c / p_up + p_c / p_do);
			Up += 2. * Dpp_c / pow2(p_c) * (p_c / p_do - 0.5 * (Dpp_up - Dpp_do) / Dpp_c - p_c / p);
...
void TimeEvolutor::evolve_in_z(const Sources& Q, const DiffusionCoefficient& D_zz, const AdvectionVelocity& u_z,
	...
	double dt_half = 0.5 * _dt;
	double dz = _z.getAverageStep();
	size_t E_size = _E.size(), z_size = _z.size();
	size_t idisc = _z.getNearestIndex(0.);
	...
	for (size_t iE = 0; iE < E_size - 1; iE++) {
		for (size_t iz = 1; iz < z_size - 1; iz++) {
			...
			const double Dz = D_zz.get(iE, iz);
			const double DzUp = (iz < z_size - 1) ? D_zz.get(iE, iz + 1) : Dz;
			const double DzDo = (iz > 0) ? D_zz.get(iE, iz - 1) : Dz; // TODO check this
			...
			rhs.at(iz - 1) = f.get(iE, iz) * (2. - central_diagonal.at(iz - 1));
			rhs.at(iz - 1) += _dt * Q.get(iE, iz) / (double) number_of_operators;
			if (iz != z_size - 2) {
				rhs.at(iz - 1) -= f.get(iE, iz + 1) * upper_diagonal.at(iz - 1);
			}
			if (iz != 1) {
				rhs.at(iz - 1) -= f.get(iE, iz - 1) * lower_diagonal.at(iz - 2);
			}
		}
